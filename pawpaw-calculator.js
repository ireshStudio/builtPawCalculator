...
.pawpaw-product-image-placeholder {
  display: grid;
  width: 100%;
  height: 100%;
  place-items: center;
  background: linear-gradient(135deg, #eef7f4 0%, #f8f2e8 100%);
  color: var(--brand-ink);
}

.pawpaw-product-image-placeholder span {
  display: grid;
  width: min(64%, 96px);
  aspect-ratio: 1;
  place-items: center;
...
`,Xi=`pawpaw-calculator-shopline`,Zi=`data-pawpaw-calculator-mounted`;function Qi(){return`
:host {
  display: block;
  width: 100%;
  color-scheme: light;
...
${Yi.replace(/^(\s*):root\s*\{/gm,`$1:host {`).replace(/^(\s*)body\s*\{/gm,`$1:host {`)}
`}function $i(){let e=document.currentScript,t=e?.dataset.target;if(t){let e=document.querySelector(t);if(e)return e}let n=e?.previousElementSibling;if(n instanceof HTMLElement&&n.id===Xi)return n;let r=document.getElementById(Xi);if(r)return r;let i=document.createElement(`div`);return i.id=Xi,e?.parentNode?e.parentNode.insertBefore(i,e):document.body.append(i),i}function ea(){let e=$i();if(e.getAttribute(Zi)===`true`)return;e.setAttribute(Zi,`true`);let t=e.shadowRoot??e.attachShadow({mode:`open`}),n=document.createElement(`style`),r=document.createElement(`div`);t.textContent=``,n.textContent=Qi(),r.setAttribute(`part`,`root`),t.append(n,r),yr(Ji,{target:r})}document.readyState===`loading`?document.addEventListener(`DOMContentLoaded`,ea,{once:!0}):ea()})();